...
with open('./output/bioembeds_model_scores_fold_'+str(test_bin)+'.json', 'w') as outfile:
...
pickle.dump(classifier, open('./models/bioembeds_fold_'+str(test_bin)+'_model.sav', 'wb'))
...
if classifier_type == 'svm':
    ...
    y_dec_score_test = classifier.predict_proba(X_test)[:,1]
print("Testing results: fold-"+str(test_bin))
print("Kappa score: "+str(cohen_kappa_score(y_test,y_pred_test)))
print(confusion_matrix(y_test, y_pred_test))
print(classification_report(y_test, y_pred_test))
np.save('./output/y_fold_'+str(test_bin)+'.npy',y_test)
np.save('./output/y_pred_fold_'+str(test_bin)+'.npy',y_pred_test)
np.save('./output/y_dec_score_fold_'+str(test_bin)+'.npy',y_dec_score_test)
